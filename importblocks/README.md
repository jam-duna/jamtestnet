...
### Quickstart: Running `importblocks` via BASH

We recommend using the [**importblocks_runner script**](https://github.com/jam-duna/jamtestnet/blob/main/importblocks/importblocks_runner.sh) provided in the [**jam-duna/testnet**](https://github.com/jam-duna/jamtestnet) repo to quickly test your team implementation:
...
JAM Duna's `importblocks` image is periodically published to [GAR](https://us-central1-docker.pkg.dev/jam-duna/importblocks/importblocks). For fine-tuning testing,  teams can pull directly from registry:
...
* **Modes covered by Feb PoC:**
  ...
  - [x] `fallback`: Generates PreStates & Blocks for extrinsic-less slots. (Validation only. Since there's nothing to fuzz.)
...
| MisclassificationRate | Misclassifications / OriginalBlocks       | Original blocks submission recognized as valid but returned with different post stateroot       |
...
| MisclassificationRate  | OverallMisclassifications / TotalBlocks                 | Blocks submission partially recognized but returned with different JamErrors or stateRoot    |
