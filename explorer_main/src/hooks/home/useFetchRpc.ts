            ...
            if (bCount === DB_LIMIT && sCount === DB_LIMIT) {
                ...
                if (response !== null) {
                    ...
                    if (lastSlotRef.current !== currentSlot) {
                        ...
                        console.log("Block Announcement: (Slot, Timestamp)", response.header.slot, response.timestamp);
                        const overview = {
                        ...
                        if (blockCount > DB_LIMIT) {
                            const oldest = await db.blocks.orderBy("overview.createdAt").first();
                            if (oldest) {
                        ...
                        if (stateCount > DB_LIMIT) {
                            const oldest = await db.states.orderBy("overview.createdAt").first();
                            if (oldest) {
